...
import asyncio,os,insepct,logging,functools
...
def has_request_arg(fn):
	...
	for name,param in params.items():
		...
		if found and (param.kind != inspect.Parameter.VAR_POSITION and param.kind != inspect.Parameter.KEYWORD_ONLY and param.kind != inspect.Parameter.VAR_KEYWORD):
			raise ValueError('request parameter must be the last named parameter function: %s%s' % (fn.__name__,str(sig)))
	return found
...
def add_route(app,fn):
	...
	logging.info('add route %s %s => %s(%s)' % (methos,path,fn.__name__,', '.join(inspect.signature(fn).parameters,keys())))
